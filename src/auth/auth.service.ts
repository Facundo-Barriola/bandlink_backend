...
export class AuthService {
  ...
  async changePassword(userId: string, dto: ChangePasswordDTO) {
    ...
    const currentPasswordOk = await bcrypt.compare(dto.currentPassword, user.password_hash);

    if (!currentPasswordOk) {
      throw new UnauthorizedException('La contraseña actual es incorrecta');
    }

    const samePassword = await bcrypt.compare(dto.newPassword, user.password_hash);
  ...
  async resendVerificationEmail(userId: string) {
    ...
    const rawToken = await this.createEmailVerificationToken(userId);
    try {
      await this.mailerService.sendVerificationMail(user.email, rawToken);
    } catch (mailError) {
      console.error('ERROR SEND VERIFICATION MAIL =>', mailError);
    }
  }
